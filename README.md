...
4. **Using the client**, fill the table:
   ...
   ('On “Good Men” and the Vague, Low Standards Required to Be One', 'Abi Maxwell', '2018-04-29', 4.5, '<p>The first time I fell in love, I was 14 years old, working nights and weekends as a ski instructor at the local mountain that had one chairlift and one rope tow. He also worked there, and he didn’t love me back—or at least I hope he didn’t; he was 25. Saying it now, as an adult, feels horrifying, though for years I stood firm that he was a good man and there was nothing particularly sinister about the relationship. He picked me up on the way to the mountain, and drove me home at night. We never actually had lessons to teach, so we spent our time riding the chairlift and skiing together. Sometimes, we would talk on the phone. He even made me a mix tape.</p>'),
...
5. Retrieve articles with the information about the author attached to each row (there should be 12 rows in the result and around 10 columns, including the article’s title, text, rating, and date as well as the author’s name and sex):
    ...
    SELECT *
    FROM articles
    LEFT JOIN authors ON articles.author = CONCAT_WS(' ', authors.first_name, authors.last_name);
    ```
...
8. Retrieve the content for the second page: articles 6 through 10 (still assuming chronological order).
    ...
    ```postgresql
    SELECT * FROM articles ORDER BY date DESC OFFSET 5 LIMIT 5;
    ```
...
10. Count the number of five-article pages required to accommodate all articles:
    ...
    SELECT CEIL(COUNT(title)::double precision / 5) FROM articles;
    ```
...
12. Count males and females among the authors. There should be two rows (for males and females) and two columns: `sex` (`F` or `M`) and `cnt` (count).
    ...
    ```postgresql
    SELECT sex AS sex, COUNT(sex) AS cnt FROM authors GROUP BY sex;
    ```
...
14. Calculate the total length of the text written by each author (count both `text` and `title`; you can keep the tags in `text` while counting):
    ...
    ```postgresql
    SELECT LENGTH(text) + LENGTH(title) AS length
    FROM articles;
    ```
...
15. Output all the authors in a random order. There should be only one column aliased `author` with the first and last name of the author concatenated (using a space, of course). The order of the rows should be different on each request:
    ...
    ```postgresql
    SELECT CONCAT_WS(' ', authors.first_name, authors.last_name) AS full_name FROM authors ORDER BY RANDOM();
    ```
...
16. "Anonymize" the authors: replace each author’s last name with the properly capitalized reverse of it. E.g., `Alofsin` should become `Nisfola`, `Esposito` should become `Otisopse`, etc.
    ...
    ```postgresql
    SELECT *, REVERSE(authors.last_name) FROM authors;
    ```
...
18. **(optional)** Delete all authors that haven’t written any articles:
    ...
    ```postgresql
    DELETE FROM authors WHERE NOT CONCAT_WS(' ', authors.first_name, authors.last_name) IN (SELECT author FROM articles);
    ```
