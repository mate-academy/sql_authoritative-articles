...
14. Calculate the total length of the text written by each author (count both `text` and `title`; you can keep the tags in `text` while counting):
    ...
    ```postgresql
    SELECT authors.first_name, authors.last_name, SUM(LENGTH(articles.text) + LENGTH(articles.title))
    FROM authors LEFT JOIN articles ON authors.id = articles.author_id GROUP BY authors.id 
    ```
